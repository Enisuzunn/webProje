...
export default function Alert({
  ...
  return (
    <div
      role="alert"
      className={['flex items-start gap-3 p-4 rounded-md border', bg, border].join(' ')}
    >
      ...
      <div className="flex-1 min-w-0">
        {title && <p className={`font-semibold leading-snug ${textColor}`}>{title}</p>}
        {children && <p className={`text-sm mt-0.5 ${textColor}`}>{children}</p>}
      </div>
